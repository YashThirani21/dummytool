...
def locate_api(api_name: str=None) -> dict:
    print("LA function called in line 303")
    """query the details of a specific api"""
    for category, tools in api_details_dict.items():
        for tool, tool_data in tools.items():
            for api in tool_data["api_list"]:
...
def sample_apis(gt_apis, num=200):
    print("SA function called in line 303")
...
rewrite_query_function = {
    'name': 'rewrite_query',
    'description': 'reformulate original query after incorporating provisional answer, solver feedback and the relevant previously selected APIs and the tools they belong to, to help the next round of tool search, by potentially ignoring components of the original query that have already been addressed, but without undoing any specific original instructions about the components yet to be solved',
    'parameters': {
        'type': 'object',
        'properties': {
...
verify_rewrite_function = {
    'name': 'verify_rewrite',
    'description': 'Compare two user queries and return True if the instructions are not contradictory',
    'parameters': {
        'type': 'object',
        'properties': {
            'rewritten_query': {'type': 'string'},
...
def query_all_tool_info(category:str, tools: list) -> list:
    # print("QATI function called in line 281")
...
def query_all_tool_info_in_category(cate):
    # print("QATIC function called in line 299")
...
def get_tool_description(category_name: str, tool_name: str) -> dict:
    # print("GTD function called in line 304")
...
def get_tools_descriptions(category_name: str, tool_list: str) -> dict:
    # print("GTD function called in line 313")
...
def Finish(newquery:str):
    """Finish the conversation"""
    return newquery

def decomposer_genie(original_query):
    messages = [{
        "role": "system",
    ...
    functions = [decompose_finish_function]
    for i in range(5):
        response = call_gpt(
                        messages=messages,
                        functions=functions
        ...
        tool_calls = response.choices[0].message.tool_calls
        print('Decomposer Thought: ', response.choices[0].message.content)
        if tool_calls:
            for tool_call in tool_calls:
                function_name = tool_call.function.name
                print('function_name: ', function_name)
                function_args = tool_call.function.arguments
                print('Decomposed query: ', function_args)
                if function_name.lower() == 'finish':
                    try:
                        new_query = Finish(**json.loads(function_args))
                        if new_query is not None:
                            return new_query, response.usage.total_tokens
                    except:
                        continue
            ...
            print('Decomposer failing Thought:', response.choices[0].message.content)
            continue
    ...
    return original_query, response.usage.total_tokens
...
decompose_finish_function = {
    'name': 'Finish',
    'description': 'Finish with the new, decomposed query. ',
    'parameters': {
        'type': 'object',
        'properties': {

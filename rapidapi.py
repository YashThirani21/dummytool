...
class rapidapi_wrapper(base_env):
    def __init__(self, query_json, tool_descriptions, retriever, args, process_id=0):
        ...
        # self.tool_root_dir = "/Users/yashthirani/Desktop/ClonedRepos/ToolLLM/ToolBench/data/toolenv/tools"
        self.toolbench_key = args.toolbench_key
        self.rapidapi_key = args.rapidapi_key
        # self.rapidapi_key = "0943df7f82msh9190858cdc55e78p197992jsn2000090a9b5b"
        self.use_rapidapi_key = args.use_rapidapi_key
        # self.use_rapidapi_key = True
        
        # print(f'use_rapidapi_key is {self.use_rapidapi_key}' )
        self.api_customization = args.api_customization
        # print(f'api_customization is {self.api_customization}' )
        # print(f'toolbench_key is {self.toolbench_key}' )
        # print(f'rapidapi_key is {self.rapidapi_key}' )
        # self.service_url = "http://39.105.143.28:8080/rapidapi"
        self.service_url = new_server_url
        ...
        # print(f'input_description set to {query_json["query"]}')
        ...
        print('Going through each function in API List')
        for k,api_json in enumerate(data_dict["api_list"]):
            standard_tool_name = tool_descriptions[k][0]

            print('\nThis is where the magic happens - converting API into an openAI function: \n')
...
You have access to the following tools: \n'''
    ...
    def build_tool_description(self, data_dict):
        print('Entering build_tool_description')
        ...
        print('Exiting build_tool_description')
    ...
    def fetch_api_json(self, query_json):
        ...
        for item in query_json["api_list"]:
            ...
            try:
                print(f'Loading Tool: {os.path.join(self.tool_root_dir, cate_name, tool_name + ".json")}')
    ...
    def api_json_to_openai_json(self, api_json, standard_tool_name):
        ...
        ############ Look into api_json again especially key api_description
            ...
            for k, function in enumerate(self.functions):
                if function["name"].endswith(action_name):
                    ...
                    if self.use_rapidapi_key or self.api_customization:
                        print(f'Rapid API IF statement - UNUSUAL')
                        ...
                        try:
                            ...
                            print("action_input_2")
                            print(action_input_2)
                            print(payload)
                            # response = requests.post(self.service_url, json=payload, headers=headers, timeout=30)
                            response = requests.post(self.service_url, json=payload, headers=headers, timeout=45)
                            # print(f'Back from Posting request!!!')
                        except Exception as e:
                            print(f"Error in posting request: {e}")
                    ...
                    while any([word in response["error"] for word in error_list]):
                        if cnt < len(self.rapidapi_key_list):
                            print(f'cnt {cnt} < len(self.rapidapi_key_list) {len(self.rapidapi_key_list)}') 
                            # if self.use_rapidapi_key or self.api_customization:
                            ...
                            print(colored(f'Entering else block in cnt < len(self.rapidapi_key_list)', 'red'))
                            break
...
class pipeline_runner:
    ...
    def method_converter(self, backbone_model, openai_key, method, env, process_id, single_chain_max_step=12, max_query_count=60, callbacks=None, messages=None):
        print("Inside method converter")
        if callbacks is None: callbacks = []
        if backbone_model == "chatgpt_function":
            print("Inside chatgpt_function")
            # model = "gpt-4-0613"
            model = solver_model

            # model = "gpt-4-deployment"
            # model = "gpt-3.5-turbo-16k-0613"
            print(f"Instantiating GPT4Function with {model} model")
            llm_forward = GPT4Function(model=model, openai_key=openai_key)
        elif backbone_model == "davinci":
            print("Inside davinci")
            model = davinci_model
            print("Instantiating Davinci")
        ...
        if method.startswith("CoT"):
            ...
            print("Instantiating single_chain")
            ...
            ######## Env contains the query, api list etc
            print("Instantiating DFS_tree_search")
            chain = DFS_tree_search(llm=llm_forward, io_func=env, process_id=process_id, callbacks=callbacks)

            # print("Starting start function")
            ######## This block creates a root node with the starting info like queries, API list etc
            ######## Then proceeds on using the llm_forward and messages to iteratively add new nodes
            result = chain.start(
                                single_chain_max_step=single_chain_max_step,
    ...
    def run_single_task(self, method, backbone_model, query_id, data_dict, args, output_dir_path, tool_des, messages, retriever=None, process_id=0, callbacks=None, server= None):
        ...
        ############## output_dir_path == result/anytoolbench
        ...
        # print("tool des IMPORTANT")
        # print(tool_des)
        
        ### This block is basically never invoked
        ...
        ##### Env contains the query, api list etc to actually generate the answer
        # print("Calling method converter")
        ...
        # print("Back after method converter")
    ...
    def run(self, task, messages):
        ...
        # print("Back after run_single_task")        
        return result

...
from anytool.verifier import check_task_solvable_by_function, check_task_solvable, check_rewrite_validity_verifier, combine_into_final_answer_function, check_rewrite_quality, check_solved_toolbench, check_solved_toolbench_rewrite, check_task_complete
...
class Query_Reformulator_Agent(Agent):
    ...
    def check_rewrite_validity(self, rewrite):
        print(colored(f'Inside check_rewrite_validity', 'green'))
        global rewrite_status, total_tokens, call_cnt
        rewrite_quality, rewrite_reason, tokens = check_rewrite_validity_verifier(self.query, self.provisional_answer, rewrite)
        print(f'Rewrite Quality: {rewrite_quality} - Reason: {rewrite_reason}')
        total_tokens += tokens
        call_cnt += 1
...
class Main_Search_Agent(Agent):
    ...
    def assign_main(self, query):
        print(colored(f'assign_main with query: {query}', 'green'))
